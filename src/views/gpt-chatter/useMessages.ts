...
export const useMessages = ()=>{
  const messages = reactive<Message[]>([
    // { role: 'assistant', content: `可以使用正则表达式 \\d+ 来匹配所有的数字。这个正则表达式表示匹配一个或多个连续的数字字符。在使用时，可以将该正则表达式应用于需要匹配数字的字符串中。以下是一个示例：\n\njavascript\nconst str = \"Hello 123 World 456\";\nconst regex = /\\d+/g;\nconst matches = str.match(regex);\nconsole.log(matches); // 输出: [ '123', '456' ]\n\n\n在上面的示例中，正则表达式 \\d+ 匹配了字符串中的数字 "123" 和 "456"。使用 match 方法可以获取到所有匹配的结果。`},
